...
function ItemRow({ item, sectionId, isFirst, isLast }: {
  ...
  return (
    <div className={`px-4 py-2.5 border-b border-border-subtle ${!item.is_active ? 'opacity-50' : ''}`}>
      <div className="flex items-center gap-2">
        ...
        {/* Unit — desktop only */}
        <span className="hidden sm:block text-xs text-fg-muted shrink-0 w-14 text-center">{item.unit}</span>

        {/* Price — desktop only */}
        <span className="hidden sm:block text-xs text-fg-secondary shrink-0 w-16 text-right">
          {item.unit_price != null ? `$${item.unit_price.toFixed(2)}` : <span className="text-fg-muted">—</span>}
        </span>
        ...
        <div className="hidden sm:flex items-center gap-1.5 shrink-0">
          ...
          <form action={toggleItemActive}>
            ...
            <button
              ...
              className={`rounded px-2 py-1 text-xs font-medium ${item.is_active ? 'text-fg-muted hover:bg-surface-raised' : 'text-accent-fg hover:bg-accent-dim'}`}
            >
...
function SectionCard({ section, isFirst, isLast }: {
  ...
  return (
    <div className={`rounded-xl border border-border bg-surface overflow-hidden ${!section.is_active ? 'opacity-60' : ''}`}>
      {/* Section header */}
      <div className="flex items-center gap-2 px-4 py-3 bg-surface-raised border-b border-border">
        ...
        <div className="flex gap-0.5 shrink-0">
          <form action={moveSectionUp}>
            ...
            <button
              ...
              className="flex items-center justify-center w-6 h-6 rounded text-fg-muted hover:text-fg-muted disabled:opacity-20"
            >
        ...
        {editingName ? (
          <form action={async (fd) => { await nameAction(fd); setEditingName(false) }} className="flex-1 flex items-center gap-2">
            ...
            <input
              ...
              required
              autoFocus
              className="flex-1 rounded-lg border border-border bg-surface px-2 py-1 text-sm text-fg font-semibold focus:border-green-600 focus:outline-none"
            />
            {nameState?.error && <span className="text-xs text-red-600">{nameState.error}</span>}
            <button type="submit" disabled={namePending} className="text-xs text-accent-fg font-medium hover:underline disabled:opacity-50">
              {namePending ? 'Saving…' : 'Save'}
            </button>
            <button type="button" onClick={() => setEditingName(false)} className="text-xs text-fg-muted hover:underline">Cancel</button>
          </form>
        ) : (
          <button
            type="button"
            onClick={() => setEditingName(true)}
            className="flex-1 text-sm font-semibold text-fg-secondary text-left hover:text-accent-fg"
          >
            {section.name}
          </button>
        )}
        ...
        <form action={toggleSectionAdminOnly}>
          ...
          <button
            ...
            className="rounded px-2.5 py-1 text-xs font-medium text-fg-muted hover:bg-skeleton"
          >
        ...
        <form action={toggleSectionActive}>
          ...
          <button
            ...
            className={`rounded px-2.5 py-1 text-xs font-medium ${section.is_active ? 'text-fg-muted hover:bg-skeleton' : 'text-accent-fg hover:bg-accent-dim'}`}
          >
      ...
      <div className="hidden sm:flex items-center gap-2 px-4 py-1.5 bg-surface-raised border-b border-border-subtle">
        ...
        <div className="flex-1 text-xs font-medium text-fg-muted">Item</div>
        <div className="w-14 text-xs font-medium text-fg-muted text-center">Unit</div>
        <div className="w-16 text-xs font-medium text-fg-muted text-right">$/unit</div>
        <div className="w-24 shrink-0" />
      </div>

      {/* Items */}
      {section.quote_template_items.length === 0 ? (
        <p className="px-4 py-3 text-sm text-fg-muted">No items yet.</p>
      ) : (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-dashed border-border bg-surface px-4 py-4">
        <p className="text-sm font-medium text-fg-secondary mb-2">Add section</p>
        <form action={addAction} className="flex gap-2">
          <input
            name="name"
            required
            placeholder="Section name"
            className="flex-1 rounded-lg border border-border bg-surface px-3 py-2 text-sm text-fg placeholder:text-fg-muted focus:border-green-600 focus:outline-none focus:ring-1 focus:ring-green-600"
          />
          <button
            type="submit"
            disabled={addPending}
