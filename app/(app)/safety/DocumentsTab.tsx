    ...
    <div className="space-y-5">
      ...
      {showUpload && (
        <div className="rounded-xl border border-stone-200 bg-white p-5 space-y-4">
          <h3 className="text-sm font-semibold text-stone-900">Upload safety document</h3>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-stone-500 uppercase tracking-wide">File *</label>
            <input
              ref={fileInputRef}
              type="file"
              onChange={e => setUploadFile(e.target.files?.[0] ?? null)}
              accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg"
              className="w-full text-sm text-stone-700 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border file:border-stone-200 file:text-xs file:font-medium file:bg-stone-50 file:text-stone-700 hover:file:bg-stone-100"
            />
          </div>
      ...
      {signingDocId && (
        <div className="rounded-xl border border-green-200 bg-green-50 p-4 space-y-3">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-green-800 uppercase tracking-wide">Notes (optional)</label>
            <textarea
              value={signNotes}
              onChange={e => setSignNotes(e.target.value)}
              placeholder="Any notes or comments…"
              rows={2}
              className="w-full rounded-lg border border-green-200 bg-white px-3 py-2 text-sm text-stone-900 placeholder:text-stone-400 focus:border-green-400 focus:outline-none resize-none"
            />
          </div>
      ...
      {docs.length === 0 ? (
        <div className="rounded-xl border border-stone-200 bg-white px-4 py-14 text-center">
          <p className="text-sm font-medium text-stone-600">No safety documents uploaded yet</p>
          {isSupervisorPlus && (
              ...
              <div key={doc.id} className="px-5 py-4 space-y-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-semibold text-stone-900 text-sm">{doc.title}</span>
                      {signed && (
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-lg border border-stone-200 px-3 py-1.5 text-xs font-medium text-stone-700 hover:bg-stone-50 disabled:opacity-60 transition-colors"
                    >
                      {isDownloading ? (
                ...
                {isSupervisorPlus && docSignoffs.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {docSignoffs.map(s => (
                      <span key={s.id} className="rounded-full bg-green-50 border border-green-100 px-2 py-0.5 text-xs text-green-700">
